...
namespace sorts {
    ...
    template<typename T>
    void selection_sort(std::vector<T>& v, bool (*comp) (T, T) = std::less<T>()) {
        unsigned sz = v.size();
    ...
    template<typename T>
    void insertion_sort(std::vector<T>& v, bool (*comp) (T, T) = std::less<T>()) {
        unsigned sz = v.size();
    ...
    template<typename T>
    void bubble_sort(std::vector<T>& v, bool (*comp) (T, T) = std::less<T>()) {
        unsigned sz = v.size();
        for (unsigned i = 0; i < sz; i++) {
            bool swapped = false;
    ...
    template<typename T>
    void merge_sort(std::vector<T>& v, bool (*comp) (T, T) = std::less<T>()) {
        if (v.size() > 1) {
    ...
    template<typename T>
    void heap_sort(std::vector<T>& v, bool (*comp) (T, T) = std::less<T>()) {
